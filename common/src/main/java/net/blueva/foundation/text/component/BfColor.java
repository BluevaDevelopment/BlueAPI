...
public final class BfColor {
    ...
    private static boolean supportsHex() {
        // Reflective, not a compile-time org.bukkit.Bukkit import: this class is
        // shared by every platform module, most of which have no such class at
        // all and are modern enough to support hex colors unconditionally.
        try {
            Class<?> bukkitClass = Class.forName("org.bukkit.Bukkit");
            Object version = bukkitClass.getMethod("getBukkitVersion").invoke(null);
            String versionString = version == null ? "" : version.toString();
            if (versionString.isEmpty()) {
                return true; // assume modern when the version string is unavailable (tests)
            }
            ...
            return true; // not running on Bukkit, or version detection failed - assume modern
        }
    }
